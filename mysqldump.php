<?php
/**
 * Modified open source Ifsnop\Mysqldump
 *
...
class Mysqldump
{
	 ...
	 */
	public function __construct( $type = 'mysql', $dumpSettings = array())
	{
	 ...
	 * @return null
	 */
	public function start($filename = 'php://output')
	{
		...
		if ( ! $this->resume ) {
			$this->compressManager->write( $this->get_dump_file_header() ); }
		...
		//	$this->compressManager->write(
		//		$this->typeAdapter->get_database_header( $this->db )
		//	);
		...
		if ( ! $this->overtime ) {
			$this->compressManager->write( $this->get_dump_file_footer() );
		}
		// Close output file
	...
	private function get_dump_file_header()
	{
		...
		$header = '-- Backup By VOGA Press Inc.' . PHP_EOL .
		'--' . PHP_EOL .
	...
	private function get_dump_file_footer()
	{
		...
		$footer .= '-- Backup by VOGA Press  EOF';
	 ...
	 * @return null
	 */
	private function get_database_structure()
	{
		// Listing all tables from database
		if ( empty($this->dumpSettings['include-tables']) ) {
			// include all tables for now, blacklisting happens later
			foreach ( $this->dbHandler->get_results( $this->typeAdapter->show_tables( $this->db ), ARRAY_N ) as $row ) {
		...
		//          foreach ( $this->dbHandler->get_results( $this->typeAdapter->show_triggers( $this->db ), ARRAY_A ) as $row ) {
		//              array_push( $this->triggers, $row['Trigger'] );
		//          }
		//      }
		if ( empty($this->dumpSettings['include-triggers']) ) {
			// include all views for now, blacklisting happens later
			...
			foreach ( $this->dbHandler->get_results( $this->typeAdapter->show_views( $this->db ), ARRAY_N ) as $row ) {
				if ( in_array( current( $row ), $this->dumpSettings['include-triggers'], true ) ) {
					...
					$elem = array_search(
						current( $row ),
						$this->dumpSettings['include-triggers']
					);
					unset($this->dumpSettings['include-triggers'][ $elem ]);
				}
			}
		}
	...
	{
		if ( ! $this->resume && ! $this->dumpSettings['no-create-info'] ) {
			$ret = '--' . PHP_EOL .
			"-- Table structure for table `$tableName`" . PHP_EOL .
			'--' . PHP_EOL . PHP_EOL;
			$stmt = $this->typeAdapter->show_create_table( $tableName );
			foreach ( $this->dbHandler->get_results( $stmt, ARRAY_A ) as $r ) {
	 ...
	 * @return null
	 */
	private function get_view_structure($viewName)
	{
		$ret = '--' . PHP_EOL .
	 ...
	 * @return null
	 */
	private function get_trigger_structure($triggerName)
	{
	...
	private function escape($tableName, $row)
	{
		...
		foreach ( $row as $colName => $colValue ) {
			if ( is_null( $colValue ) ) {
				...
				if ( 'bit' == $columnTypes[ $colName ]['type'] || ! empty($colValue) ) {
					$ret[] = "0x${colValue}";
				} else {
					$ret[] = "''";
				}
				...
				$ret[] = $this->dbHandler->prepare( '%s', $colValue );
			}
		}
		return $ret;
	}
	...
	{
		if ( ! $this->resume ) {
			$this->prepare_list_values( $tableName ); }
	 ...
	 * @return null
	 */
	function prepare_list_values($tableName)
	{
		$this->compressManager->write(
			'--' . PHP_EOL .
	 ...
	 * @return null
	 */
	function end_list_values($tableName)
	{
		if ( $this->dumpSettings['disable-keys'] ) {
			$this->compressManager->write(
	 ...
	 */
	function get_column_stmt($tableName)
	{
		$colStmt = array();
...
abstract class CompressMethod
{
	 ...
	 * @return boolean
	 */
	public static function is_valid($c)
	{
...
abstract class TypeAdapterFactory
{
	 ...
	 * @return array
	 */
	public function parse_column_type($colType)
	{
		return array();
	}
...
class TypeAdapterMysql extends TypeAdapterFactory
{
	...
	public function databases()
	{
		...
		$resultSet = $this->dbHandler->get_row( "SHOW VARIABLES LIKE 'character_set_database';", ARRAY_N );
		$characterSet = $resultSet[1];

		$resultSet = $this->dbHandler->get_row( "SHOW VARIABLES LIKE 'collation_database';", ARRAY_N );
		$collationDb = $resultSet[1];
		$ret = '';
	...
	public function show_views()
	{
		...
		return 'SELECT TABLE_NAME AS tbl_name ' .
		'FROM INFORMATION_SCHEMA.TABLES ' .
		"WHERE TABLE_TYPE='VIEW' AND TABLE_SCHEMA='${args[0]}' ORDER BY TABLE_NAME";
	}
